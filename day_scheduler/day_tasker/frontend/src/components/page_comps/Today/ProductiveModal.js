...
export class ProductiveModal extends Component{
    constructor(props){
        ...
        var masterTasks = this.props.tasks.map(task => {
            return(
                <TaskHighlight completedClass=' ' key={task.props.accessKey} accessKey={task.props.accessKey} name={task.props.name} time={task.props.time}/>
    ...
    updateTasks(){
        ...
        if(showTasksCopy.length > 1){
            ...
            this.setState({
                ...
                taskTime: this.timeToSeconds(newTask.props.time),
    ...
    render(){
        ...
        return(
            <div className="productive-modal"> 
                <button id="close-modal" onClick={ () => this.props.setParentState({showModal: false, showModalBackground: false})}>
